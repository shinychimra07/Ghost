...
export class AdapterManager<Registry extends AdapterRegistry = {}> {
     ...
     * accumulate the full set of known adapter types at the type level.
     *
    ...
    registerAdapter<Type extends string, Base extends AdapterConstructor>(
        type: Type,
        BaseClass: Base
    ): AdapterManager<Registry & Record<Type, InstanceType<Base>>> {
        this.instanceCache[type] = {};
        this.baseClasses[type] = BaseClass;

        return this as unknown as AdapterManager<Registry & Record<Type, InstanceType<Base>>>;
    }
